...
| 5 | Choropleth map panel — layout, legend ("Fewer → More" swatches) | PASS (layout only — see note below) |
...
**Known limitation (environment-specific, not a regression):** the world-atlas TopoJSON (`cdn.jsdelivr.net`) is blocked by this sandbox's outbound network policy, so the map polygons themselves render blank in this environment. Confirmed this is unrelated to the new code — the pre-existing `/map` page (issue's prior art, same `GEO_URL`) shows the identical blank map under the same sandbox. Both will render normally wherever that CDN is reachable (this is not a restricted host in normal dev/CI/prod environments — see `docs/features/world-map.md`).
